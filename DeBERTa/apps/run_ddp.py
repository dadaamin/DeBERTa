...
from ..training import initialize_distributed, batch_to, set_random_seed,kill_children
...
def train_model_ddp(args, model, train_data, eval_data, run_eval_fn, train_fn=None, loss_fn=None):
...
def run_predict(args, model, device, eval_data, prefix=None):
  ...
  for eval_item in eval_data:
    ...
    model.eval()
    predicts = []
    for batch in tqdm(eval_dataloader, ncols=80, desc='Evaluating: {}'.format(prefix), disable=args.rank>0):
      batch = batch_to(batch, device)
      with torch.no_grad():
        output = model(**batch)
      logits = output['logits']
...
def main(args):
  ...
  if args.do_train:
    ...
    train_model_ddp(args, model, train_data, eval_data, run_eval_fn, loss_fn=loss_fn, train_fn = train_fn)
